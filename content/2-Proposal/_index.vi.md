...
| AWS WAF (phạm vi CloudFront + phạm vi Regional) | Hai Web ACL riêng biệt bảo vệ CloudFront và endpoint API Gateway. |
...
* Giai đoạn 1. Thiết lập môi trường & Container: Cấu hình IAM/AWS CLI, xây dựng Docker/Playwright runner (Dockerfile, entrypoint.js,playwright.config.js) phụ trách
* Giai đoạn 2. Luồng sự kiện & Coordinator: Thiết lập SQS + DLQ và Lambda Coordinator gọi ECS RunTask phụ trách
...
Trong tuần 8, nhóm vừa tự học vừa tự tìm hiểu, tham khảo các blog để đề xuất với nhau, cùng nhau đánh giá và đưa ra các ý tưởng cho đề tài workshop.
...
| **Cộng dồn (các dịch vụ trên Calculator)** | — | **7.19** |
| NAT Gateway (lên lịch tạo/xóa để tối ưu chi phí) | Chỉ chạy trong khung giờ cần gọi OpenAI API, không chạy 24/7 | 15.045 |
